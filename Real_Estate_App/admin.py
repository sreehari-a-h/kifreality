...
class PropertiesAdmin(admin.ModelAdmin):
    ...
    def fetch_properties(self, request, queryset):
        try:
            ...
            data = response.json()
            for property_data in data:
                Properties.objects.create(title=property_data['title'], description=property_data['description'], price=property_data['price'])
            self.message_user(request, "Properties fetched successfully.")
        except requests.exceptions.RequestException as e:
...
class PropertiesExampleAdmin(admin.ModelAdmin):
    ...
    def fetch_properties(self, request, queryset):
        try:
            ...
            data = response.json()
            for property_data in data:
                PropertiesExample.objects.create(
                    title=property_data['title'],
                    status=property_data['status'],
...
admin.site.register(PropertiesExample, PropertiesExampleAdmin)
